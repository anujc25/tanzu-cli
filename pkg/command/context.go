...
func globalLogin(c *configtypes.Context) (err error) {
	...
	// format
	fmt.Println()
	log.Info("successfully created a TMC context")
	return nil
}
...
func selfManagedTMCLogin(c *configtypes.Context) (err error) {
	...
	// format
	fmt.Println()
	log.Info("successfully created a TMC self-managed context")
	return nil
}
...
func k8sLogin(c *configtypes.Context) error {
	if c.ClusterOpts.Path != "" && c.ClusterOpts.Context != "" {
		...
		log.Infof("successfully created a kubernetes context using the kubeconfig %s", c.ClusterOpts.Path)
		return nil
	}
