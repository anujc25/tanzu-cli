...
func buildPlugin(path, id string) (plugin, error) {
	...
	target, err := getPluginTarget(path, desc.Name, id)
	if err != nil {
		return plugin{}, err
	}
...
func getPluginTarget(pluginPath, name, id string) (string, error) {
	if !groupByOSArch {
		return "", nil
	}
	...
	if err != nil {
		log.Errorf("%s - plugin %q requires a metadata.yaml file", id, name)
		return "", err
	}
